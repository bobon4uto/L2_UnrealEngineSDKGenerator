// EXAMPLE:
// grow every time after geting a soul
// grow by defines how much target scale increases,
// threshold defines how much bigget target has to be than actual size for actual to startr catching up
// This means ~ 10 proks will give one big growth
#define GROW_BY 1.01
...
void mod_hook_before(UObject *pThis, UFunction *Function, void *Parms,
  ...
  if ( pThis->IsA(AGameInfo::StaticClass()) ) {
    ...
    if (self->Owner && !is(fn,"GetEffTargetLocation") ) {
      //cout << name << "->" << fn << "|" << self->Owner->GetFullName() << "\n";
      if (
             is(name,"s_u809_absorb_soul_ca") // this is absorb soul
          && is(fn,"EndState")
          ) {
        MetaActor* self_ma = get_ma(self->Owner);
        self_ma->target_scale *= GROW_BY;
      }
    }
  }
