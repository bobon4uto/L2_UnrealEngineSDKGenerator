// EXAMPLE:
// character grows by 1.1 upon lvl up
// would be nice to be able to get lvl exactly,
// here we only can know that lvl up occured, but no info on char lvl
...
void mod_hook_before(UObject *pThis, UFunction *Function, void *Parms,
  ...
  if ( pThis->IsA(AGameInfo::StaticClass()) ) {
    ...
    if (self->Owner) {
      if (
             is(name,"e_u004_a") // this is lvl up animation
          && is(fn,".BeginPlay")
          ) {
        MetaActor* self_ma = get_ma(self->Owner);
        self_ma->target_scale *= 1.1;
      }
    }
  }
